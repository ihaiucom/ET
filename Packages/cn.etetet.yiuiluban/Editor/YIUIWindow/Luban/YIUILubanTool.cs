...
namespace YIUILuban.Editor
{
    public class YIUILubanTool : OdinMenuEditorWindow
    {
        ...
        {
            LubanEditorData = OdinSerializationUtility.Load<LubanEditorSerializationData>(LubanEditorSerializationData.LubanEditorPackageSettingsPath) ?? new();
        ...
        protected override void OnDestroy()
        {
            ...
            OdinSerializationUtility.Save(LubanEditorData, LubanEditorSerializationData.LubanEditorPackageSettingsPath);
        }
        ...
        [Button("仅生成Conf", 50)]
        [ShowIf(nameof(GenPackageExists))]
        public void CreateLubanConf()
        {
            var result = LubanTools.CreateLubanConf();
            YIUIFramework.UnityTipsHelper.Show($"Luban生成Conf {(result ? "成功" : "失败")}");
        }
